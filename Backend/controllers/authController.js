...
export const register = async (req, res) => {
  try {
    ...
    const existingUser = await User.findOne({
      $or: [{ email }, { username }],
      isDeleted: false, // Ensure we don't find deleted users
    });
    ...
    const user = await User.create({
      ...
      role: "user",
      isApproved: false,
      isVerified: false, // Initially set isVerified to false
    });
    ...
    await sendEmail(email, "Email Verification", "Verify your email", htmlContent);

    res.status(201).json({
      status: "success",
      message: "User registered. Awaiting approval. Please check your email to verify your account.",
      data: {
        id: user._id,
        email: user.email,
...
export const requestPasswordReset = async (req, res) => {
  ...
  try {
    ...
    await sendEmail(email, "Password Reset", "Reset your password", htmlContent);
